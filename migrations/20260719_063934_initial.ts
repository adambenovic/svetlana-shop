...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."_locales" AS ENUM('sk', 'en', 'cs', 'de', 'es', 'fr', 'hu', 'it', 'pl', 'uk');
  CREATE TYPE "public"."enum_products_currency" AS ENUM('EUR', 'CZK');
  ...
  CREATE TABLE "products" (
  	...
  	"has_bg" boolean DEFAULT false,
  	"parts_key" varchar,
  	"status" "enum_products_status" DEFAULT 'draft',
  	"configurator_only" boolean DEFAULT false,
  ...
  CREATE TABLE "orders" (
  	...
  	"shipping_packeta_point_id" numeric,
  	"shipping_packeta_point_name" varchar,
  	"shipping_packeta_point_city" varchar,
  	"total_amount" numeric NOT NULL,
  	"currency" varchar DEFAULT 'EUR',
  	"gopay_id" varchar,
  	"packeta_id" varchar,
  	"shipment_error" varchar,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
